...
SINR = [15,20,25,30,35,40,45,50,55]; % Signal-to-Interference-plus-Noise Ratio in linear scale
...
% SNR Calculation
SNR1a = 10*log10(mean(dataBits/demodulatedBits));
SNR1b = find(SNR1a ~= 0 & SNR1a ~= -inf);
SNR1 = abs(SNR1a(SNR1b(1)))
...
% SNR Calculation
SNR2a = 10*log10(mean(dataBits./demodulatedBits));
SNR2b = find(SNR2a ~= 0 & SNR2a ~= -inf);
SNR2 = abs(SNR1a(SNR1b(1)))/2
...
% SNR Calculation
SNR3a = 10*log10(mean(dataBits./demodulatedBits));
SNR3b = find(SNR3a ~= 0 & SNR3a ~= -inf);
SNR3 = abs(SNR1a(SNR1b(1)))/3
...
% SNR Calculation
SNR4a = 10*log10(mean(data/decodedData));
SNR4b = find(SNR4a ~= 0 & SNR4a ~= -inf);
SNR4 = abs(SNR4a(SNR4b(1)))
...
% SNR Calculation
SNR5a = 10*log10(mean(dataLW/deinterleavedDataLW));
SNR5b = find(SNR5a ~= 0 & SNR5a ~= -inf);
SNR5 = abs(SNR5a(SNR5b(1)))
...
% SNR Calculation
SNR6a = 10*log10(mean(dataCS/decodedDataCS));
SNR6b = find(SNR6a ~= 0 & SNR6a ~= -inf);
SNR6 = abs(SNR6a(SNR6b(1)))
...
% SNR Calculation
SNR7a = 10*log10(mean(dataCH/demodulatedBits));
SNR7b = find(SNR7a ~= 0 & SNR7a ~= -inf);
SNR7 = abs(SNR7a(SNR7b(1)))
...
% With Turbo Coding
% for inip = 1:7
numBits = 1024;          % Number of input bits
...
% Turbo Encoding
trellis = poly2trellis(4, [13 15], 13); % Turbo encoder trellis
interleaverIndices = randperm(numBits); % Random interleaver
turboEncoder = comm.TurboEncoder('TrellisStructure', trellis, 'InterleaverIndices', interleaverIndices);
encodedBits = turboEncoder(dataBits);

% QPSK Modulation
qpskModulator = comm.QPSKModulator('BitInput', true);
modulatedSymbols = qpskModulator(encodedBits);

% Transmit through AWGN channel
txSignal = modulatedSymbols;
noisePower = 10^(-snr/10) / 2;
awgnNoise = sqrt(noisePower) * (randn(size(txSignal)) + 1j * randn(size(txSignal)));
rxSignal = txSignal + awgnNoise;

% Soft Detection at the Receiver
demodulator = comm.QPSKDemodulator('BitOutput', true, 'DecisionMethod', 'Approximate log-likelihood ratio', ...
                                   'Variance', noisePower);
softBits = demodulator(rxSignal);

% Turbo Decoding
turboDecoder = comm.TurboDecoder('TrellisStructure', trellis, 'InterleaverIndices', interleaverIndices, ...
                                  'NumIterations', 6);
decodedBits = turboDecoder(softBits);

% BER Calculation
[~, berT] = biterr(dataBits, decodedBits);
% BERT(inip) = berT;
% Spectral Efficiency
SE8 = log2(1 + SINR(8)); % bits/s/Hz

% Data Rate
R8 = B * SE8; % bits/s

% Latency
T_t8 = packetSize ./ R8; % Transmission delay
T_latency8 = T_p + T_proc + T_t8 + T_q; % Total latency

% SNR Calculation
SNR8a = 10*log10(mean(dataBits/decodedBits));
SNR8b = find(SNR8a ~= 0 & SNR8a ~= -inf);
SNR8 = abs(abs(SNR8a(SNR8b(1))))

disp(['Spectral Efficiency8: ', num2str(SE8), ' bits/s/Hz']);
disp(['Data Rate8: ', num2str(R8/1e6), ' Mbps']);
disp(['Latency8: ', num2str(T_latency8*1e3), ' ms']);
% end

%% With Polar Coding
% for inip = 1:7
numBits = 1024;          % Number of input bits
N = 1024;                % Polar code length (must be a power of 2)
K = 512;                 % Number of information bits
snr = 5;                 % Signal-to-Noise Ratio in dB

% Generate random binary data
dataBits = [];
dataBits = randi([0 1], K, 1);

% Polar Encoding (Manual)
frozenBits = zeros(N, 1); % Frozen bit indices
frozenBits(end-K+1:end) = 1; % Indices of information bits
u = zeros(N, 1);
u(frozenBits == 1) = dataBits; % Assign information bits
encodedBits = polarTransform(u);

% Interleaving
interleaverIndices = randperm(length(encodedBits)); % Random interleaver
interleavedBits = encodedBits(interleaverIndices);
...
% De-interleaving
deinterleavedBits = zeros(size(softBits));
deinterleavedBits(interleaverIndices) = softBits;

% Polar Decoding (Manual Successive Cancellation)
decodedBits = polarDecode(deinterleavedBits, frozenBits);

% BER Calculation
[~, berP] = biterr(dataBits, decodedBits);
% BERP(inip) = berP;
% Spectral Efficiency
SE9 = log2(1 + SINR(9)); % bits/s/Hz

% Data Rate
R9 = B * SE9; % bits/s

% Latency
T_t9 = packetSize ./ R9; % Transmission delay
T_latency9 = T_p + T_proc + T_t9 + T_q; % Total latency

% SNR Calculation
SNR9a = 10*log10(mean(dataBits/decodedBits));
SNR9b = find(SNR9a ~= 0 & SNR9a ~= -inf);
SNR9 = abs(SNR9a(SNR9b(1)))

disp(['Spectral Efficiency9: ', num2str(SE9), ' bits/s/Hz']);
disp(['Data Rate9: ', num2str(R9/1e6), ' Mbps']);
disp(['Latency9: ', num2str(T_latency9*1e3), ' ms']);
% end
...
load('BER3.mat');load('BERT.mat');load('BERP.mat');

% Average BER
avBERL = mean(BERL);avBERLW = mean(BERLW);
avBERCS = mean(BERCS);avBERCH = mean(BERCH);
avBERT = mean(BERT);avBERP = mean(BERP);
...
figure;
semilogy(snrdb,sort(BERT,'descend'),'r->','LineWidth',2);
hold on;
semilogy(snrdb,sort(BERLW,'descend'),'k->','LineWidth',2);
xlabel('SNR (dB)');
ylabel('BER');
title('Bit error Rate Comparsion of Turbo Coding');
legend('QPSK With Turbo','QPSK Without Turbo')

figure;
semilogy(snrdb,sort(BERP,'descend'),'r->','LineWidth',2);
hold on;
semilogy(snrdb,sort(BERLW,'descend'),'k->','LineWidth',2);
xlabel('SNR (dB)');
ylabel('BER');
title('Bit error Rate Comparsion of Polar Coding');
legend('QPSK With Polar','QPSK Without Polar')

%% Functions
% --- Polar Transform ---
function encoded = polarTransform(u)
    N = length(u);
    n = log2(N);
    encoded = u;
    for stage = 1:n
        ...
        for i = 1:step:N
            for j = 0:(step/2-1)
                encoded(i+j) = mod(encoded(i+j) + encoded(i+j+step/2), 2);
            end
        end
    end
end
% --- Polar Decoding ---
function decoded = polarDecode(llr, frozenBits)
    N = length(llr);
    n = log2(N);
    u_hat = zeros(N, 1);
    for i = 1:N
        if frozenBits(i) == 0
            ...
            u_hat(i) = llr(i) < 0; % Decision based on LLR
        end
    end
    decoded = polarTransform(u_hat); % Inverse polar transform
    decoded = decoded(end - sum(frozenBits) + 1:end); % Extract information bits
end